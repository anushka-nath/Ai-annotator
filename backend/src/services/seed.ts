...
export function buildSeedTasks(total = 40): Array<{ type: TaskType; content: string }> {
  const rows: Array<{ type: TaskType; content: string }> = [];
  for (let i = 0; i < total; i += 1) {
    const type: TaskType = i % 2 === 0 ? "text" : "image";
    const content =
      type === "text"
