...
apiRouter.get("/tasks", async (req, res, next) => {
  try {
    ...
    if (q.assignee) filters.assignedTo = q.assignee;
...
apiRouter.post("/tasks/seed", async (_req, res, next) => {
  try {
    ...
    const tasks = buildSeedTasks(40).map((t) => ({ ...t, status: "unassigned" as const }));
...
apiRouter.post("/tasks/reset", async (_req, res, next) => {
  try {
    await AnnotationModel.deleteMany({});
    const result = await TaskModel.updateMany(
      {},
      { $set: { status: "unassigned" }, $unset: { assignedTo: 1 } }
    );
...
apiRouter.post("/annotate", async (req, res, next) => {
  try {
    ...
    const annotation = await AnnotationModel.findOneAndUpdate(
      { taskId: task._id },
      {
        taskId: task._id,
        label: input.label,
